...
##########CONSTANTS######################################
 .equ UNAVAILABLE, 0     #This is the number we will use to mark
			 #space that has been give out
 .equ ALAILABLE, 1       #This is the numbe we will use to mark
...
#
#RETURN VALUES: This function returns the address of the 
...
allo_loop_begin:               #here we iterate through each 
			       #memory region
...
move_break:			#If we've made it here, that 
 ...
 pushl %ecx                     
 pushl %ebx
 movl $SYS_BRK, %eax            #reset the break(%ebp has the
 ...
 int $LINUX_SYSCALL             
				...
				#isn't 0, we don't care what it it
 cmpl $0, %eax  		#check for error condition
 je error

 popl %ebx                      #restor saved registers
 popl %ecx
 popl %eax

#set this memory as unavailable, sine we're about to give it away
 movl $UNAVAILABLE, HDR_AVAIL_OFFSET(%eax)
#set the size of the memory
 movl %ecx, HDR_SIZE_OFFSET(%eax)
#move the %eax to the actual start of the uable memory.
