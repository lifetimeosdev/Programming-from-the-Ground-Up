...
# This program will compute the value of the 2^3+5^2
...
popl %ebx       #The second answer is already in 
                #%eax. We saved the first answer onto the stack,
		#so now we can just pop it out into %ebx
	
addl %eax, %ebx #add the together. the result is in %ebx
...
power_loop_start:
cmpl $1, %ecx       #if the power is 1,we have done
je end_power
movl -4(%ebp), %eax #move the current value into %eax
